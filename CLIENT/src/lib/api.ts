...
let toeknGetter: (() => Promise<string | null>) | null = null;
...
api.interceptors.request.use(async (config) => {
  if (!toeknGetter) return config;

  const token = toeknGetter();
...
function getErrorMsg(error: unknown) {
  ...
  return "Something wentt wrong!!! Please try again";
}
...
export async function getApi<T>(url: string, config?: AxiosRequestConfig) {
  try {
    ...
    if (response.data.status === "error" || !response.data.data) {
      throw new Error(response.data.error?.[0]?.message || "Request failed");
    }
